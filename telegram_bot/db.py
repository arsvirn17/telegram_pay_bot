...
class Database:
    ...
    def edit_users(self, user_id, money) -> None:
        self.cursor.execute('UPDATE `users` SET `money` = ? WHERE `user_id` = ?', (money, user_id,))

    def add_block_user(self, user_is):
        with self.connection:
            return self.cursor.execute("INSERT INTO `blockusers` (`user_is`) VALUES (?)", (user_is,)) 

    def get_block(self):
        with self.connection:
            result = self.cursor.execute("SELECT * FROM 'blockusers'").fetchall()       
            return result 